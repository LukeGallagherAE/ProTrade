...
export default function NewInvoicePage() {
  ...
  const [form, setForm] = useState({
    ...
    taxRate: 10, dueDate: addDays(todayISO(), 14),
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (res.ok) { const inv = await res.json(); showToast('Invoice created'); router.push(`/invoices/${inv.id}`); }
    else { showToast('Failed to create invoice', 'error'); setSaving(false); }
  }
      ...
      <form onSubmit={submit} className="space-y-4">
        <Card>
          ...
          <div className="space-y-4">
            <Input label="Invoice Title *" value={form.title} onChange={e => setField('title', e.target.value)} placeholder="e.g. Hot Water System Replacement" required />
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-50">
                {items.map(item => (
                  <tr key={item.id}>
                    <td className="px-4 py-2"><input value={item.description} onChange={e => updateItem(item.id, 'description', e.target.value)} className="w-full rounded border border-slate-200 px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-brand-500" placeholder="Item description" /></td>
                    <td className="px-4 py-2"><input type="number" min="0" step="0.01" value={item.quantity} onChange={e => updateItem(item.id, 'quantity', parseFloat(e.target.value) || 0)} className="w-full rounded border border-slate-200 px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-brand-500" /></td>
                    <td className="px-4 py-2"><input value={item.unit} onChange={e => updateItem(item.id, 'unit', e.target.value)} className="w-full rounded border border-slate-200 px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-brand-500" /></td>
                    <td className="px-4 py-2"><input type="number" min="0" step="0.01" value={item.unitPrice} onChange={e => updateItem(item.id, 'unitPrice', parseFloat(e.target.value) || 0)} className="w-full rounded border border-slate-200 px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-brand-500" /></td>
                    <td className="px-4 py-2 font-medium text-slate-700">{formatCurrency(item.total)}</td>
                    <td className="px-2 py-2"><button type="button" onClick={() => setItems(p => p.filter(i => i.id !== item.id))} className="p-1 text-slate-400 hover:text-red-500 transition-colors"><Trash2 size={14} /></button></td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div className="w-64 space-y-2 text-sm">
              ...
              <div className="flex justify-between items-center text-slate-600">
                ...
                <div className="flex items-center gap-2">
                  <input type="number" value={form.taxRate} onChange={e => setField('taxRate', parseFloat(e.target.value) || 0)} className="w-14 rounded border border-slate-200 px-2 py-1 text-xs text-right focus:outline-none focus:ring-1 focus:ring-brand-500" />
                  <span className="text-xs">% = {formatCurrency(tax)}</span>
                </div>
              </div>
              <div className="flex justify-between font-bold text-slate-900 text-base border-t border-slate-200 pt-2"><span>Total</span><span>{formatCurrency(total)}</span></div>
            </div>
          </div>
        </Card>
        ...
        <Card>
          <div className="space-y-4">
            <Textarea label="Notes" value={form.notes} onChange={e => setField('notes', e.target.value)} rows={2} placeholder="Payment instructions, bank details, etc." />
